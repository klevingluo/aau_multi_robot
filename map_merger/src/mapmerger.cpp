...
MapMerger::MapMerger() {    
  ...
  nodeHandle->param<int>("seconds_send_position",seconds_send_position,10);
...
/**
 * registers callbacks for new robots
 */
void MapMerger::waitForRobotInformation() {
  ...
  ros::Subscriber sub =nodeHandle->subscribe(
      ...
      1000,
      &MapMerger::callback_got_robot_for_data,
      this);
...
// called on other robots entering range
void MapMerger::callback_new_robot(const std_msgs::StringConstPtr &msg) {
  ...
  for(int i = 0; i < robots->size(); i++) {
    if(robots->at(i) == newRobotName) {
      ...
      UpdateEntry* lastUpdate = update_list->back();
  ...
  // we should initialize all robot specific data here, so that we don't have to keep track of different arrays in different indices
  robots->push_back(newRobotName);
  new_data_maps->push_back(false);
  transforms->push_back(Mat(0,0, CV_64F, double(0)));

  robots_in_transform->push_back(newRobotName);
...
// responds to a map request by sending the local map, the whole thing
...
/**
 * merge the maps into the local map, then publish to global map
 */
void MapMerger::callback_global_pub(const ros::TimerEvent &e) {
  ...
  {
    ROS_WARN("no local map data");
    return;
  }

  if(robots->size() == 0) {
    ROS_INFO("No maps from other robots, publishing local map");
    pub.publish(*local_map);
    return;
  }

  // this is where transforms are computed
  ROS_INFO("parameters: %lu, %lu, %lu", 
      transforms->size(), 
      map_data->size(), 
      robots->size());

  if(transforms->size() != map_data->size()) {
    for(size_t i = 0; i < robots->size(); i++) {
      if(findTransformIndex(i) != -1) {
        ROS_INFO("Computing Transform, robotIndex:%i", static_cast<int>(i));
        recomputeTransform(findTransformIndex(i));
      }
    }
  }
  ...
  if(map_data->size() > 0 && transforms->size() > 0) {
    ...
    for(int i = 0; i < transforms->size(); i++) {
      ...
      if(transforms->at(findRobotIndex(i)).total() <= 0) {
        ROS_INFO("Skipping %s, transform not computed",
            robots->at(findRobotIndex(i)).c_str());
        continue;
      }
...
void MapMerger::callback_send_map(const ros::TimerEvent &e)
{
  ...
  {
    ROS_DEBUG("Do not send map, local map contains no data");
    return;
  }
  ...
  max_x = map_width;
  max_y = map_height;
  ROS_FATAL("Does this ever get called?");
#endif
...
void MapMerger::processLocalMap(nav_msgs::OccupancyGrid * toInsert)
{
  ...
  // copy the local map to the global map if one doesn't exist
  if(global_map == NULL)
  {
...
void MapMerger::start()
{
  ...
  ros::Subscriber sub_other_maps = nodeHandle->subscribe(
      ...
      1000,&MapMerger::callback_map_other,
      this);
  ...
  sub_position_local = nodeHandle->subscribe(
      robot_prefix+ "/" + position_local_robot_topic,
      1000,
      &MapMerger::callback_got_position,
      this);

  sub_position_network = nodeHandle->subscribe(
      robot_prefix + "/" + position_other_robots_topic,
      5,
  ...
  my_pos_pub = nodeHandle->advertise<visualization_msgs::MarkerArray>(
      "position_"+robot_name,
      3);

  // because i hat to send them in a occupancy grid, i send them as int, so i lose the numbers after the comma
  ...
  if(seconds_recompute_transform != -1) {
    ...
    recompute_transform_timer = nodeHandle->createTimer(
        ...
        &MapMerger::callback_recompute_transform,
        this);
  }
...
nav_msgs::OccupancyGrid* MapMerger::matToMap(const Mat mat, nav_msgs::OccupancyGrid *forInfo)
{
  ...
  for(int i=2; i<toReturn->info.height * toReturn->info.width;i++)
  {
    if(mat.data[i]== 254)//KNOWN
      toReturn->data[i]=0;

    else if(mat.data[i] < 50) toReturn->data[i]=100; //WALL
...
/// adds map to the merged global map
/// I think this only updates the size
...
/**
 * recomputes the transform indexed mapDataIndex
 */ 
bool MapMerger::recomputeTransform(int mapDataIndex) {
  ...
  if(transIndex == -1)
  {
    StitchedMap mapStiched(img_local,img_other,max_trans_robot,max_rotation_robot,max_pairwise_distance,cv::Mat());
  ...
  {
    Mat cur_trans;
    if (transforms->at(transIndex).total() > 0) {
      cur_trans = transforms->at(transIndex);
    } else {
      cur_trans = cv::Mat();
    }

    StitchedMap mapStiched(img_local,img_other,max_trans_robot,max_rotation_robot,max_pairwise_distance,cur_trans);
    worked = mapStiched.works;
    if(worked)
    {
      newTrans = mapStiched.H;
...
/// pushes an empty map onto the lest of maps, updates the position if necessary
/// @param robot_name - the name of the other robot
...
/**
 * ahh ok, so instead of sending the whole map, just send updates every time.  I'm sure there's a better way to abstract this though
 */
void MapMerger::sendMapOverNetwork(string destination, std::vector<int>* containedUpdates, int start_row, int start_collum, int end_row, int end_collum) {
  ...
  ROS_DEBUG("Send map using:%s \n\t\t\t\t tlldiwway min_x:%i|max_x:%i|min_y:%i|max_y:%i",service.c_str(),start_row,end_row,start_collum,end_collum);
  /**
   * todo:
  ...
  for(int row = start_row; row < end_row;row+=size)
  {
    for(int collum = start_collum; collum < end_collum;collum+=size)
    {
      ...
      ROS_DEBUG("Send Map to %s(if nothing -> broadcast) ,frame id:%s| topic:[%s]|data<_size:%lu",
...
/**
 * finds the index of the transform corresponding to the robot index
 */
...
void MapMerger::sendControlMessage(std::vector<int>* updateNumbers,std::string dest) {
  ...
  {
    if(exchange.response.status)
      ROS_DEBUG("Sent control message");
    else
...
/**
 * ask for map updates from other robots periodically
 */
